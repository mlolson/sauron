...
import type { BackgroundJob, JobTrigger, JobWorkspace, Project } from '@shared/types'
import { WorkspaceSelect } from './WorkspaceSelect'
import { describeTrigger } from '@shared/jobs'
import { TriggerEditor } from './TriggerEditor'

/** Sets when and where an attached background agent runs for this project, or hands either back to the agent's default. */
export function TriggerDialog({ project, job, defaultTrigger, defaultWorkspace, onClose }: {
  project: Project
  job: BackgroundJob
  /** The agent's own trigger and workspace, used when the project has no override. */
  defaultTrigger: JobTrigger
  defaultWorkspace: JobWorkspace
  onClose: () => void
}) {
  const [custom, setCustom] = useState(job.customTrigger)
  const [trigger, setTrigger] = useState<JobTrigger>(job.trigger)
  const [workspace, setWorkspace] = useState<JobWorkspace | undefined>((project.backgroundJobs ?? []).find((j) => j.template === job.id)?.workspace)
  const [error, setError] = useState<string | null>(null)
  const valid = !custom || ((trigger.kind !== 'cron' || trigger.schedule) && (trigger.kind !== 'interval' || trigger.every >= 1))

  const submit = () => {
    const next = (project.backgroundJobs ?? []).map((j) => (j.template === job.id ? { ...j, trigger: custom ? trigger : undefined, workspace } : j))
  ...
  return (
    <div className="modal-backdrop" onMouseDown={onClose}>
      <div className="modal job-dialog" onMouseDown={(e) => e.stopPropagation()} onKeyDown={(e) => e.key === 'Escape' && onClose()}>
        <header><h2>{job.name} · settings for {project.name}</h2></header>
        ...
        <WorkspaceSelect value={workspace} onChange={setWorkspace} allowDefault defaultValue={defaultWorkspace} />
